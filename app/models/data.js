...
// **********************************************
	...
	// Update Folder
	this.updateFolder = function (inFolderObject) {
		...
		this.db.transaction((function (inTransaction) {
			inTransaction.executeSql(sqlUpdateFolder, 
				[f.label, f.privy, f.archived, f.sortorder, f.modified, f.value, f.id],
				function() { },
				this.errorHandler);
	    }).bind(this));
...
// **********************************************
	...
	// Update Context
	this.updateContext = function (inContextObject) {
		...
		this.db.transaction((function (inTransaction) {
			inTransaction.executeSql(sqlUpdateContext, 
				[c.label, c.modified, c.value, c.id],
				function() { },
				this.errorHandler);
	    }).bind(this));
...
// **********************************************
	...
	// Update Goal
	this.updateGoal = function (g) {
		//Mojo.Log.info("Entering db updateGoal()");
		
		this.db.transaction((function (inTransaction) {
			inTransaction.executeSql(sqlUpdateGoal, 
				[g.label, g.level, g.archived, g.contributes, g.modified, 
					g.value, g.id],
				function() { },
				this.errorHandler);
	    }).bind(this));
...
// **********************************************
	...
	this.deleteDeletedTask = function (inTaskValue) {
		//Mojo.Log.info("Entering db deleteFolder()");
		// Delete Folder with value inFolder
		this.db.transaction((function (inTransaction) {
...
*/
	...
	var sqlUpdateTasksWithFolder = "UPDATE 'tasks' SET folder=0 WHERE folder=?; GO;";
	var sqlUpdateTasksWithContext = "UPDATE 'tasks' SET context=0 WHERE context=?; GO;";
